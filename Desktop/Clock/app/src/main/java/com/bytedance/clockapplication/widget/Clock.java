...
public class Clock extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mHourPaint.setColor(DEFAULT_PRIMARY_COLOR);
        mHourPaint.setTextSize(40f);
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        HOUR_POINTER_LENGTH = PANEL_RADIUS - 80;
        MINUTE_POINTER_LENGTH = PANEL_RADIUS - 50;
        SECOND_POINTER_LENGTH = PANEL_RADIUS - 30;
    ...
    private void drawHoursValues(Canvas canvas) {
        ...
        for (int i = 1; i <= 12; i++) {
            float angle = (float) Math.toRadians((i - 3) * 30);
            float x = (float) (mCenterX + r * Math.cos(angle));
            float y = (float) (mCenterY + r * Math.sin(angle) + (mHourPaint.getTextSize() / 3));
            canvas.drawText(String.valueOf(i), x, y, mHourPaint);
        }
    }
